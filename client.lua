...
RegisterNetEvent('tra_tv:update')
AddEventHandler('tra_tv:update', function(players)
...
CreateThread(function()
    ...
    TriggerServerEvent('tra_tv:fetch')
...
RegisterCommand('tv', function(source, args)
    if args[1] then
        for k, v in pairs(Config['Objects']) do
            ...
            if DoesEntityExist(obj) then
                TriggerServerEvent('tra_tv:add', args[1], v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'])
                break
            end
        end
    end
            ...
            if volume >= 0 then
                ...
                for k, v in pairs(data) do
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                        ...
                        TriggerServerEvent('tra_tv:setvolume', k, volume)
                        break
                    end
                end
...
RegisterCommand('destroy', function(src, args)
    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
            TriggerServerEvent('tra_tv:destroy', k)
            break
        end
    end
end)

RegisterNetEvent('tra_tv:delete')
AddEventHandler('tra_tv:delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('tra_tv:updatevolume')
AddEventHandler('tra_tv:updatevolume', function(id, volume)
    if data[id] then
